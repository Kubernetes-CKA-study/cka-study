...
사용자 이름, 비밀번호, 토큰을 일반 텍스트 파일에 저장하는 방식은 보안상 안전하지 않기 때문에 권장 X
### 2. 인증서 기반 인증
#### TLS in Kubernetes
![img1](img/img1.png)
인증서 종류
...
관리자가 직접 CA 키가 있는 서버에 로그인해 CSR을 서명 → 번거로움 → Certificate API(CSR API) 이용

Certificate API(CSR API) : 인증서를 **API 기반으로 관리**하는 방법
1. 사용자는 개인 키 생성, CSR을 생성
2. 관리자는 이 CSR을 base64로 인코딩, 클러스터에 CSR 오브젝트 생성
3. 관리자는 `kubectl get csr`로 요청을 확인, `kubectl certificate approve`로 승인
4. 쿠버네티스가 내부 CA 키를 사용해 인증서에 서명
5. 서명된 인증서는 CSR 오브젝트에서 base64 디코딩해 추출한 뒤 사용자에게 전달
→ CA 키 노출 x
→ 인증서 발급/갱신을 오브젝트로 관리 가능
→ 확장이 손 쉬움

내부 구조
- 인증서 서명, 관리는 `kube-controller-manager`가 담당
- 이 매니저에 CA 인증서, 키 경로 지정되어 있음
- 관리자는 '승인', 컨트롤 플레인이 '서명'

##### KubeConfig
KubeConfig : 
- kubectl이 사용할 클러스터/사용자/컨텍스트 설정 파일
- API 서버 주소, 인증서/키, 사용자 권한 정보를 한 곳에 저장
→ 키파일 옵션을 지정하는 것은 번거로움 → kubeconfig 설정파일에 지정

![img4](img/img4.png)

구성 요소
1. clusters : 접근할 수 있는 클러스터 목록
2. users : 접근 사용자 목록, 클라이언트 인증서와 개인 키 지정
3. contexts : cluster + user 연결

**current-context**
- kubeconfig에는 현재 사용 중인 컨텍스트가 하나 지정됨
- 모든 kubectl 명령은 이 컨텍스트를 기준으로 실행됨
<br>

- 인증서 경로는 루트부터 적는 것이 좋음
- 네임스페이스 지정 가능
- 이미 존재하는 사용자/권한을 가지고 어떤 클러스터에 어떤 사용자로 접근할지 매핑

- `kubectl config view` : 현재 config 파일 확인
- `kubectl config use-context <context명>` : context 전환
- `kubectl --kubeconfig=/path/to/config get pods` : 다른 kubeconfig 파일 사용

## B. Authorization : 권한
권한 : 접근한 사용자가 무엇을 할 수 있는지를 정의

### API Groups
- 모든 Kubernetes 작업은 **kube-apiserver**를 통해 수행됨
- `kubectl` 명령이나 REST 호출은 모두 API 서버와 통신
- Kubernetes 리소스는 API 그룹 단위로 관리

Kubernetes API는 **목적과 기능**에 따라 여러 그룹으로 나뉨

- Core Group (/api/v1)
  - Kubernetes의 핵심 기능
  - Namespace, Pod, Service, ConfigMap, Secret, Node, Event, Endpoint, PV, PVC 등
- Named Groups (/apis/<group>/<version>)
  - 기능별로 명확하게 분리됨
  - 최신 기능은 대부분 여기 포함됨

API Groups 확인 방법
- 클러스터의 API 서버(:6443)에 경로 없이 접속하면 → 지원되는 API 그룹 목록이 표시
- `curl https://<API-SERVER>:6443`

**리소스 + 동사 (Verb)**
각 리소스는 수행 가능한 **동작(verbs)** 를 가짐
- get, list, create, delete, update, watch 등
**RBAC는 “API 그룹 + 리소스 + verb” 조합으로 동작**
- 어떤 API 그룹의 어떤 리소스에 어떤 verb를 허용할지 를 정의함

**kube-proxy vs kubectl proxy**

kube-proxy
- 클러스터 내부 컴포넌트
- Service ↔ Pod 네트워크 연결 담당

kubectl proxy

- 로컬 개발/관리용 HTTP 프록시
- kubectl이 API 서버에 대신 요청 전달
...
#### 1. 노드 권한 부여자(Node Authorizer)
- 클러스터 내부 통신에 사용
- Kubelet이 노드 권한 부여자에 의해 승인되는 조건 → `system:nodes` 그룹에 속하고 이름에 `system:node` 접두사가 붙어야 함
...
#### 3. 역할 기반 액세스 제어(RBAC)  
- 역할(Role)에 권한 규칙을 정의한 뒤 사용자를 역할에 연결
...
// no
```
관리자는 실제로 해당 사용자로 로그인하지 않아도, --as 옵션을 사용해 다른 사용자를 가장하여 권한 테스트 가능
#### 4. Webhook 방식
- 권한 부여를 외부 시스템에 위임
...
> - NetworkPolicy는 정책, 실제 차단은 CNI 플러그인(Calico/Cilium 등)이 함
