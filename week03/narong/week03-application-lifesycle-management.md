...
쿠버네티스에서 리소스를 확장하는 방법은 크게 방향(수직/수평)과 대상(인프라/워크로드)에 따라 나뉩니다.

## 5.1. 스케일링의 기본 개념: 수직 vs 수평

### 수직 스케일링 (Vertical Scaling)

- **개념:** 기존 서버의 사양(Spec)을 높이는 것입니다. (Scale Up)
- **비유:** 더 튼튼하고 힘센 사람으로 교체하는 것.
- **방법:** CPU, 메모리(RAM)를 추가합니다.

### 수평 스케일링 (Horizontal Scaling)

- **개념:** 서버의 개수를 늘리는 것입니다. (Scale Out)
- **비유:** 평범한 사람 여러 명을 투입해서 일을 분담하는 것.
- **방법:** 서버(노드)를 추가하거나 파드를 추가합니다.

## 5.2. 스케일링의 대상: 인프라 vs 워크로드

쿠버네티스에서는 스케일링 대상을 크게 두 가지 층위로 나눕니다.

### 1) 인프라 (Infrastructure) - 노드(Node) 레벨

물리 서버나 가상 머신(VM) 자체를 늘리거나 줄이는 것입니다.

- **수평 스케일링:** 노드 개수 추가 (`kubeadm join` 등)
- **수직 스케일링:** 기존 노드의 CPU/RAM 증설 (하지만 보통은 고사양의 새 노드로 교체하는 방식을 씁니다. 운영 중인 서버 사양을 바꾸는 건 어렵기 때문입니다.)

### 2) 워크로드 (Workload) - 파드(Pod) 레벨

애플리케이션이 실행되는 컨테이너(파드)를 늘리거나 줄이는 것입니다.

- **수평 스케일링:** 파드 개수(Replicas) 증가
- **수직 스케일링:** 파드에 할당된 리소스 제한 증가

## 5.3. 스케일링 방법: 수동 vs 자동

### 수동 스케일링 (Manual Scaling)

관리자가 직접 명령어를 입력하여 조절하는 방식입니다.

- **인프라:** `kubeadm join` 명령어로 노드 추가 프로비저닝.
- **워크로드 (수평):** `kubectl scale` 명령어로 레플리카 수 조정.
...
- **워크로드 (수직):** `kubectl edit` 명령어로 파드 정의 파일의 리소스(`resources`) 수정. (전통적으로는 파드 재시작이 필요함)

# 6. 자동 스케일링 (Automated Scaling)
...
## 6.6. Vertical Pod Autoscaler (VPA)
